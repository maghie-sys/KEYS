...
Future<List<Recommand>> fetchRecommand() async {
  final response = await http.get(Uri.parse('http://localhost:8000/keys/api/home/2/'));
  if (response.statusCode == 200) {
    //final Recommands = jsonDecode(response.body);
    //print(Recommands[0]["Th_Img1"]);
    List list = jsonDecode(response.body);
    var recommandList = list.map((element) => Recommand.fromJson(element)).toList();
    return recommandList;
    //return Recommand.fromJson(jsonDecode(response.body)[0]);
...
/*
List<Recommand> parseRecommands(String responseBody) {
  final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
  return parsed.map<Recommand>((json) => Recommand.fromJson(json)).toList();
}
*/
                        ...
                        Expanded(
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Card(
                                  child:ListTile(
                                    title : Text(recommand.Th_Img1),
                                  )

                                /*ListView(//포스터 출력부분. list view로 출력하며 좌우 스크롤
                                  scrollDirection: Axis.horizontal,
                                  children: List.generate(
                                    posters.length, (index) =>
                                      Poster(
                                          posterUrl: recommand.Th_Img1),) //posters[index]),)//위에 리스트로 만든 poster url값을 poster.dart로 넘겨줌
                                ),
