...
from .serializers import home_recommand_se, show_review_se, show_theme_se, show_image_se
...
@api_view(['GET'])
def show_review(request,re_id):
    #thgrs = ThGr.objects.filter(Th_CODE=id)
    thgrd = ThGr.objects.filter(id=re_id)
    serializer = show_review_se(thgrd, many=True)
    return  Response(serializer.data)

@api_view(['GET'])
def show_image(request,re_id):
    img = ThImg.objects.filter(id=re_id)
...
def th_index(request):
    ...
    return render(request, 'theme/th_list.html', context)
...
def th_detail(request, th_id):
    ...
    context = {'th': th}
    return render(request, 'theme/th_detail.html', context)
...
def theme_create(request):
    ...
    if request.method=='POST':
        ...
        form = ThemeForm()
    context = {'form': form}
    return render(request, 'th/theme_form.html' , {'form':form})
...
def review_create(request, th_id):
    ...
    #review = ThGr(th=th, Th_CODE=th_code, ThGr_pt=0, ThGr_review=request.POST.get('review'))
    #review.save()
    return redirect('th:detail', th_id=th.id)
